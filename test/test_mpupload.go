...
func main() {
	...
	filehash := "ba3e0abef16b7ed559c6fb12ccc75445887a014e"
	filesize := fmt.Sprintf("%d", 17510)

	// 1. 请求初始化分块上传接口
	resp, err := http.PostForm(
		"http://10.16.69.126:62200/file/mpupload/init",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {filehash},
	...
	// 3. 请求分块上传接口
	filename := "./fastdfs-nginx-module_v1.16.tar.gz"
	tURL := "http://10.16.69.126:62200/file/mpupload/uppart?" +
		"username=admin&token=" + token + "&uploadid=" + uploadID
	multipartUpload(filename, tURL, chunkSize)

	// 4. 请求分块完成接口
	resp, err = http.PostForm(
		"http://10.16.69.126:62200/file/mpupload/complete",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {filehash},
